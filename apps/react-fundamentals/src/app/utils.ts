...
export interface TechMeta { label: string; icon: IconType; color: string }
...
export const TDT_CATEGORIES: TdtCategory[] = ['react-hooks', 'architecture'];
...
import type { FindAllQcmQuestionsQuery } from '@mas/react-fundamentals-sot';

type GqlQuestion = FindAllQcmQuestionsQuery['findAllQcmQuestion'][number];

export function toFlatQuestion(q: GqlQuestion): FlatQuestion {
  return {
    id:          q.id,
    moduleId:    q.moduleId,
