 ...
 *
 * Accessible at `/profile` (auth-guarded). Shows a {@link ProfileCard} by default;
 * switches to {@link ProfileForm} or {@link ChangePasswordForm} on user action.
 *
 * Mutation logic mirrors the auth route's profile section but lives here so the
 * page is reachable under the app shell with breadcrumbs.
 */
...
import { Container, Stack, Spinner } from '@mas/react-ui';
...
export function ProfilePage() {
  ...
  // Merge fresh GQL data over auth.identity so ProfileForm always has latest values
  ...
  const handleChangePassword = async (_current: string, _next: string) => {
    setError(null);
    // TODO: wire to changePassword mutation when backend supports it
    setMode('view');
  };
