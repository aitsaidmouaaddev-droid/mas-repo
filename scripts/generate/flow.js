...
const {
  section, summaryBox, onCancel, buildFlagsString,
} = require('./utils');
...
function assertCleanWorkingTree() {
  try {
    ...
    if (status) {
      console.log('');
      console.log(chalk.bold.red('╔══════════════════════════════════════════════╗'));
      console.log(chalk.bold.red('║') + chalk.bold.white('   🚫  Uncommitted changes detected           ') + chalk.bold.red('║'));
      console.log(chalk.bold.red('╚══════════════════════════════════════════════╝'));
      console.log('');
      ...
      console.log('');
      console.log(chalk.bold('  Please commit or stash your changes before generating a new project.'));
...
async function runFlow(opts = {}) {
  ...
  const { name } = await prompts({
    ...
    validate: v => /^[a-z][a-z0-9-]*$/.test(v) ? true : 'Use lowercase letters, numbers and dashes only',
  }, { onCancel });
