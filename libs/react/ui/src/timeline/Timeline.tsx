 ...
 *
 * @property animate - When true, each item fades in line-by-line as it enters the viewport
  ...
  return (
    ...
    >
      ...
      {items.map((entry, index) => {
        const isVisible = !animate || !!visibleItems[index];
        // delay step between rows: 80ms
        let delay = 0;
        const fd = (d: number) => ({ '--fade-delay': `${d}ms` }) as React.CSSProperties;
        ...
        return (
          <div
            ...
            data-idx={index}
            className={clsx(s.className.item, isVisible && animate && scss.itemVisible)}
            style={s.style.item}
          >
            <div className={s.className.dot} style={s.style.dot} />

            <h3
              className={clsx(s.className.title, animate && scss.fadeItem)}
              style={{ ...s.style.title, ...(animate ? fd((delay += 0)) : {}) }}
            >
              {entry.title}
            </h3>
            ...
            {entry.subtitle && (
              <span
                className={clsx(s.className.subtitle, animate && scss.fadeItem)}
                style={{ ...s.style.subtitle, ...(animate ? fd((delay += 120)) : {}) }}
              >
                {entry.subtitle}
              </span>
            )}
            ...
            {entry.location && (
              <p
                className={clsx(s.className.location, animate && scss.fadeItem)}
                style={{ ...s.style.location, ...(animate ? fd((delay += 120)) : {}) }}
              >
                {entry.location}
              </p>
            )}
            ...
            {entry.description && (
              <p
                className={clsx(s.className.description, animate && scss.fadeItem)}
                style={{ ...s.style.description, ...(animate ? fd((delay += 120)) : {}) }}
              >
                {entry.description}
              </p>
            )}
            ...
            {entry.bullets && entry.bullets.length > 0 && (
              <ul className={s.className.bullets} style={s.style.bullets}>
                {entry.bullets.map((text, i) => (
                  <li
                    key={i}
                    className={clsx(s.className.bullet, animate && scss.fadeItem)}
                    style={{ ...s.style.bullet, ...(animate ? fd((delay += 80)) : {}) }}
                  >
                    {text}
                  </li>
                ))}
            ...
            {entry.subItems && entry.subItems.length > 0 && (
              <div
                className={clsx(s.className.subItems, animate && scss.fadeItem)}
                style={{ ...s.style.subItems, ...(animate ? fd((delay += 120)) : {}) }}
              >
